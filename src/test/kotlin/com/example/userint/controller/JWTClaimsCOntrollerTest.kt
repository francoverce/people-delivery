package com.example.userint.controller

import com.example.userint.domain.entities.Claim
import com.example.userint.domain.model.UpdateTicket
import com.example.userint.domain.requests.ClaimDTO
import com.example.userint.domain.requests.CreateReviewsRequest
import com.example.userint.v1.JWTClaimsController
import org.assertj.core.api.Assertions
import org.junit.jupiter.api.Test
import org.mockito.Mock
import org.springframework.beans.factory.annotation.Autowired
import org.springframework.boot.test.context.SpringBootTest
import org.springframework.http.HttpHeaders
import org.springframework.http.HttpStatus
import org.springframework.http.ResponseEntity
import org.springframework.mock.web.MockHttpServletRequest
import java.util.*
import javax.servlet.http.HttpServletRequest

@SpringBootTest
class JWTClaimsCOntrollerTest {
    @Autowired
    lateinit var jwtClaimsController: JWTClaimsController

    @Mock
    lateinit var mockRequest: HttpServletRequest

    val authorizationHeader = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9..WuRPeW98uLH6whXW5ZaFHbDcfssfuYE1DYXYpsB_ax_A_d0Olj6qV5HfQJeX3A0cpzS9kXAeq-7RJE6A4lOBkQ"
    val tripCode: UUID = UUID.fromString("7191ec49-9f2e-4a35-bac5-f4d6e3448c9a")
    val idClaim: Long = 28

    @Test
    fun createClaim() {
        val  claimDTO = ClaimDTO(
            driverName = "ConductorTest",
            tripCode = tripCode,
            title = "ReclamoTest",
            description = "Es un reclamo de prueba"
        )

        // Datos esperados
        val expectedResponse = ResponseEntity<CreateReviewsRequest>(HttpStatus.CREATED)


        // Crear una instancia de HttpServletRequest simulada
        val simulatedRequest = MockHttpServletRequest()

        // Establecer el encabezado de autorización en la solicitud simulada
        simulatedRequest.addHeader(HttpHeaders.AUTHORIZATION, authorizationHeader)

        // Ejecutar la llamada al método con la solicitud simulada
        val response = jwtClaimsController.createClaim(simulatedRequest, claimDTO)

        Assertions.assertThat(response.statusCode).isEqualTo(expectedResponse.statusCode)

    }

    @Test
    fun getClaims() {
        // Datos esperados
        val expectedResponse = ResponseEntity<CreateReviewsRequest>(HttpStatus.OK)


        // Crear una instancia de HttpServletRequest simulada
        val simulatedRequest = MockHttpServletRequest()

        // Establecer el encabezado de autorización en la solicitud simulada
        simulatedRequest.addHeader(HttpHeaders.AUTHORIZATION, authorizationHeader)

        // Ejecutar la llamada al método con la solicitud simulada
        val response = jwtClaimsController.getClaim(simulatedRequest)

        Assertions.assertThat(response.statusCode).isEqualTo(expectedResponse.statusCode)
        Assertions.assertThat(response.body as List<Claim>).isNotEmpty()
    }

    @Test
    fun updateClaims() {
        // Datos esperados
        val expectedResponse = ResponseEntity<CreateReviewsRequest>(HttpStatus.OK)

        val updateTicket = UpdateTicket(
            idTicket = idClaim,
            newStatus = "OPEN",
            timestampActualizacion = "2023-11-19T13:43:00Z"
        )

        // Crear una instancia de HttpServletRequest simulada
        val simulatedRequest = MockHttpServletRequest()

        // Establecer el encabezado de autorización en la solicitud simulada
        simulatedRequest.addHeader(HttpHeaders.AUTHORIZATION, authorizationHeader)

        // Ejecutar la llamada al método con la solicitud simulada
        val response = jwtClaimsController.updateClaim(simulatedRequest,updateTicket)

        Assertions.assertThat(response.statusCode).isEqualTo(expectedResponse.statusCode)
    }

}